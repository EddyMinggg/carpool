    ...
    @if(!$isMobile)
        ...
        <div style="display: flex; gap: 2rem; margin-bottom: 1.5rem;">
            ...
            <div class="stats-card-green" style="flex: 1;">
                <div class="flex items-center justify-between">
                    <div>
                        <p style="color: rgba(255, 255, 255, 0.8); font-size: 0.875rem; font-weight: 500; margin-bottom: 0.25rem;">Active Users</p>
            ...
            <div class="stats-card-purple" style="flex: 1;">
                <div class="flex items-center justify-between">
                    <div>
                        <p style="color: rgba(255, 255, 255, 0.8); font-size: 0.875rem; font-weight: 500; margin-bottom: 0.25rem;">New This Month</p>
                                ...
                                <td>
                                    <div class="flex space-x-1">
                                        ...
                                        @if(Auth::user()->id !== $user->id && !(Auth::user()->is_admin === 1 && $user->is_admin === 2))
                                            <button onclick="showDeleteModal(document.getElementById('delete-form-{{ $user->id }}'), '{{ $user->username }}')" 
                    ...
                    <div class="mobile-stats-grid">
                        ...
                        <div class="mobile-stat-card blue">
                            <div style="display: flex; flex-direction: column; align-items: center; text-align: center;">
                                ...
                                <p style="color: rgba(255,255,255,0.8); font-size: 10px; font-weight: 500; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 4px;">Total</p>
                                <p style="font-size: 20px; font-weight: bold; margin: 0;" id="mobile-total-users">{{ $users->count() }}</p>
                            </div>
                        </div>
                        ...
                        <div class="mobile-stat-card green">
                            <div style="display: flex; flex-direction: column; align-items: center; text-align: center;">
                                ...
                                <p style="color: rgba(255,255,255,0.8); font-size: 10px; font-weight: 500; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 4px;">Active</p>
                                <p style="font-size: 20px; font-weight: bold; margin: 0;" id="mobile-active-users">{{ $users->whereNotNull('email_verified_at')->count() }}</p>
                            </div>
                        </div>
                        ...
                        <div class="mobile-stat-card purple">
                            <div style="display: flex; flex-direction: column; align-items: center; text-align: center;">
                                ...
                                <p style="color: rgba(255,255,255,0.8); font-size: 10px; font-weight: 500; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 4px;">New</p>
                                <p style="font-size: 20px; font-weight: bold; margin: 0;" id="mobile-new-users">{{ $users->where('created_at', '>=', now()->startOfMonth())->count() }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div style="padding: 16px; box-sizing: border-box;">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 12px;" id="mobile-users-container">
                        @foreach($users as $user)
                            <a href="{{ route('admin.users.show', $user->id) }}" 
                               ...
                               data-role="{{ $user->is_admin === 2 ? 'Super Admin' : ($user->is_admin === 1 ? 'Admin' : 'User') }}" 
                               data-username="{{ strtolower($user->username) }}" 
                               data-email="{{ strtolower($user->email) }}"
                               style="text-decoration: none; display: block; transition: all 0.2s ease;">
                                <div style="display: flex; align-items: center; gap: 12px;">
                                    <!-- 頭像 -->
                                    <div class="avatar {{ $user->is_admin === 2 ? 'avatar-super' : ($user->is_admin === 1 ? 'avatar-admin' : 'avatar-user') }}">
                                    ...
                                    <div style="flex: 1; min-width: 0; overflow: hidden;">
                                        <div style="display: flex; align-items: center; margin-bottom: 6px;">
                                            <h3 style="font-weight: 600; color: #1f2937; font-size: 16px; margin: 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin-right: 8px;">{{ $user->username }}</h3>
                                            <span class="role-badge {{ $user->is_admin === 2 ? 'role-super' : ($user->is_admin === 1 ? 'role-admin' : 'role-user') }}">
                                                {{ $user->is_admin === 2 ? 'SUPER' : ($user->is_admin === 1 ? 'ADMIN' : 'USER') }}
                                            </span>
                                        </div>
                                        <p style="font-size: 14px; color: #6b7280; margin: 0 0 4px 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ $user->email }}</p>
                                        @if($user->phone)
                                            <p style="font-size: 12px; color: #9ca3af; margin: 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ $user->phone }}</p>
                                        @endif
                                    </div>
                ...
                @else
                    ...
                    <div style="text-align: center; padding: 48px 16px;">
                        ...
                        <h3 style="font-size: 18px; font-weight: 600; color: #4b5563; margin-bottom: 8px;">No Users Found</h3>
                        <p style="color: #6b7280; margin-bottom: 24px; font-size: 14px;">There are currently no users in the system.</p>
                    </div>
                @endif
            </div>
