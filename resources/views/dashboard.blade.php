    ...
    <div x-data="{
        ...
        <div class="relative px-4 pt-4 pb-2">
            <div class="flex items-center justify-between mb-3">
                ...
                <button @click="showDatePicker = !showDatePicker"
                    class="flex items-center gap-2 bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-600 transition-colors">
                    <span class="material-icons text-base text-gray-500 dark:text-gray-400">calendar_today</span>
                    <span class="text-sm text-gray-700 dark:text-gray-300 font-medium">{{ __('Pick Date') }}</span>
                </button>
            </div>
        ...
        <div x-show="showDatePicker" 
            ...
            <div class="bg-white rounded-lg shadow-xl max-w-sm w-full mx-4 p-6">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-semibold text-gray-900">{{ __('Select Date') }}</h3>
                ...
                <div class="text-center mb-4">
                    <h4 class="text-lg font-semibold text-gray-900">{{ __('2025 October') }}</h4>
                </div>
                ...
                <div class="grid grid-cols-7 gap-1">
                    <template x-for="day in calendarDays" :key="day.date">
                        <button 
                            @click="day.selectable && selectDateFromCalendar(day.date)"
                            :disabled="!day.selectable"
                            :class="{
                                // 當前選中的日期
                                'bg-blue-600 text-white': day.date === activeDate,
                                
                                // 有班車的日期（綠色）
                                'bg-green-100 text-green-800 border-2 border-green-400 hover:bg-green-200': day.dateType === 'has-trips' && day.date !== activeDate,
                                
                                // 2週內但沒有班車（橙色邊框）
                                'bg-orange-50 text-orange-600 border-2 border-orange-300 cursor-not-allowed': day.dateType === 'no-trips-in-range',
                                
                                // 超出範圍或其他月份（只用灰色文字）
                                'text-gray-400 cursor-not-allowed': day.dateType === 'out-of-range' || day.dateType === 'other-month'
                            }"
            ...
            <div class="flex gap-3 overflow-x-auto no-scrollbar snap-x snap-mandatory pb-2"
                ...
                <template x-for="(date, index) in dates" :key="date">
                    <button
                        ...
                        :class="activeDate === date ? 'bg-blue-600 dark:bg-blue-700 text-white shadow-lg scale-105 border-blue-600 dark:border-blue-700' :
                        ...
                        class="min-w-[80px] px-3 py-3 rounded-xl font-semibold text-sm transition-all duration-300 snap-center focus:outline-none whitespace-nowrap border-2 shadow-sm">
                        <div class="w-full text-xs opacity-75" x-text="new Date(date).toLocaleDateString('en', {weekday: 'short'})"></div>
        ...
        <div class="flex flex-col gap-3 px-4 py-4">
            <template x-for="trip in trips" :key="trip.id">
                <div 
                    :class="{
                        'bg-gradient-to-br from-yellow-400 via-yellow-500 to-amber-500 border-yellow-600 shadow-yellow-200': trip.type === 'golden',
                        'bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700': trip.type !== 'golden' && !trip.is_expired,
                        'bg-gray-100 dark:bg-gray-900 border-gray-300 dark:border-gray-600 opacity-70': trip.type !== 'golden' && trip.is_expired
                    }"
                    :class="(trip.type !== 'golden' && trip.is_expired) ? 'cursor-not-allowed' : 'cursor-pointer'"
                    class="rounded-xl p-4 sm:p-6 shadow-md border transition-all"
                    :class="!(trip.type !== 'golden' && trip.is_expired) ? 'hover:shadow-lg hover:scale-[1.02] active:scale-98' : ''"
                    @click="!(trip.type !== 'golden' && trip.is_expired) ? (window.location='/trips/' + trip.id) : null">
                    
                    <!-- Golden Hour 特殊標記 -->
                    <template x-if="trip.type === 'golden'">
                        <div class="flex items-center justify-center mb-3">
                            <div class="bg-white/20 backdrop-blur-sm text-white px-4 py-2 rounded-full text-sm font-bold shadow-lg flex items-center gap-2 border border-white/30">
                                <span class="material-icons text-base">star</span>
                                <span>{{ __('Golden Hour') }} - {{ __('Instant Departure!') }}</span>
                                <span class="material-icons text-base">star</span>
                            </div>
                        </div>
                    </template>
                    ...
                    <div class="flex justify-between items-start mb-3">
                        <div class="flex-1">
                            <div 
                                :class="{
                                    'text-white': trip.type === 'golden',
                                    'text-gray-900 dark:text-gray-100': trip.type !== 'golden' && !trip.is_expired,
                                    'text-gray-500 dark:text-gray-400': trip.type !== 'golden' && trip.is_expired
                                }"
                                class="text-2xl sm:text-3xl font-bold"
                                x-text="trip.formatted_departure_time">
                            </div>
                            <!-- 普通時段類型標籤 (Golden hour 已在上方顯示) -->
                            <template x-if="trip.type !== 'golden'">
                                <div class="mt-1">
                                    <span 
                                        :class="{
                                            'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300': trip.type === 'normal' && !trip.is_expired,
                                            'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-300': trip.type === 'fixed' && !trip.is_expired,
                                            'bg-gray-200 dark:bg-gray-600 text-gray-600 dark:text-gray-400': trip.is_expired
                                        }"
                                        class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium">
                                        <span class="material-icons text-xs mr-1" 
                                            x-text="trip.type === 'fixed' ? 'attach_money' : (trip.is_expired ? 'schedule_send' : 'schedule')">
                                        </span>
                                        <span x-text="trip.is_expired ? '{{ __('Booking Closed') }}' : trip.type_label"></span>
                                    </span>
                                </div>
                            </template>
                        </div>
                        <div class="text-right ml-4">
                            <div 
                                :class="{
                                    'text-white': trip.type === 'golden',
                                    'text-blue-600 dark:text-blue-400': trip.type !== 'golden' && !trip.is_expired,
                                    'text-gray-500 dark:text-gray-400': trip.type !== 'golden' && trip.is_expired
                                }"
                            ...
                            <div 
                                :class="{
                                    'text-white/80': trip.type === 'golden',
                                    'text-gray-500 dark:text-gray-400': trip.type !== 'golden'
                                }"
                                class="text-xs"
                                x-text="'{{ __('per person') }}'">
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="flex-1">
                            <div class="flex items-center gap-2">
                                <span 
                                    :class="{
                                        'text-white': trip.type === 'golden',
                                        'text-gray-400 dark:text-gray-500': trip.type !== 'golden' && !trip.is_expired,
                                        'text-gray-500 dark:text-gray-600': trip.type !== 'golden' && trip.is_expired
                                    }"
                                    class="material-icons text-base">place</span>
                                <div 
                                    :class="{
                                        'text-white': trip.type === 'golden',
                                        'text-gray-700 dark:text-gray-300': trip.type !== 'golden' && !trip.is_expired,
                                        'text-gray-500 dark:text-gray-400': trip.type !== 'golden' && trip.is_expired
                                    }"
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-4">
                            ...
                            <div class="flex items-center gap-1">
                                <span 
                                    ...
                                    class="material-icons text-sm">info</span>
                                <span 
                                    :class="{
                                        'text-white/90': trip.type === 'golden',
                                        'text-gray-500 dark:text-gray-400': trip.type !== 'golden'
                                    }"
                                    class="text-xs"
                            ...
                            <template x-if="trip.type === 'normal' && trip.four_person_discount > 0 && !trip.is_expired">
                                <span class="bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 px-2 py-1 rounded-full text-xs font-medium">
                        ...
                        <div 
                            ...
                            x-text="trip.is_expired ? '{{ __('Booking Closed') }}' : '{{ __('Tap to join') }}'">
                        </div>
                    </div>
                </div>
            </template>
